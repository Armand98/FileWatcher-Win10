...
namespace FileWatcher
{
    ...
    class FileScanner
    {
        ...
        private static string reverseDate(string date)
        {
            ...
            reversedString = date.Substring(6, 4) + "/" + date.Substring(3, 2) + "/" + date.Substring(0, 2) + date.Substring(10, 9);
        ...
        private static void insert(FileSystemInfo fsi, OracleConnection connection)
        {
            ...
            string statement = "INSERT INTO SINGLE_SCANS(FILE_NAME, FILE_EXTENSION, FILE_PATH, LAST_ACCESS, LAST_WRITE, CREATION_DATE)" +
                              $" VALUES('{fsi.Name}', '{fsi.Extension}', '{fsi.FullName}', TO_DATE('{reverseDate(fsi.LastAccessTime.ToString())}', 'yyyy/mm/dd hh24:mi:ss'), " +
                              $"TO_DATE('{reverseDate(fsi.LastWriteTime.ToString())}', 'yyyy/mm/dd hh24:mi:ss'), TO_DATE('{reverseDate(fsi.CreationTime.ToString())}', 'yyyy/mm/dd hh24:mi:ss'))";
